...
- order: `function` *(optional)*
  - The custom sorting function. Sorts the keys in ascending order if none is provided.

**Example**
```lua
local fruits = { "apple", "orange", "banana" }

local sortedByKeyDesc = ESX.Table.Sort(fruits, function(t, a, b)
    return a > b
end)

for key, value in sortedByKeyDesc do

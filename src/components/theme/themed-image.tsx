...
export function ThemedImage({ type = "header" }) {
  ...
  // Memoize theme calculation to avoid recalculations
  const _theme = useMemo(() => 
    theme === "system" ? systemTheme : theme
  , [theme, systemTheme]);

  // Memoize images object to avoid recreation on each render
  const images = useMemo(() => ({
    ...
    RTXDEV: {
      light: "https://esx.s3.fr-par.scw.cloud/rtx-logo-black.png",
  ...
  // Return placeholder with fixed height to prevent layout shift
  if (!mounted) {
    return <div style={{ height: type === "header" ? "60px" : "100px" }} />;
  }
